...
void performDelaunayTriangulation(std::vector<Point>& points, std::vector<Triangle>& triangles) {
    triangles.clear();
                    ...
                    if (event.mouseButton.button == sf::Mouse::Left) {
                        if (event.mouseButton.x >= 10 && event.mouseButton.x <= 210) {
                            if (event.mouseButton.y >= 10 && event.mouseButton.y <= 60) {
                                // Clear previous points and triangles
                                points.clear();
                                triangles.clear();
                                // Generate random points
                                std::vector<Point> randomPoints = generateRandomPoints(numOfPoints, 0, window_width, 0, window_height);
                                points = randomPoints; // Use these points for Delaunay Triangulation
                                // Perform Delaunay Triangulation
//                                buildSequentialTriangles(points, triangles);
                                performDelaunayTriangulation(points, triangles);
                    ...
                    if (event.mouseButton.button == sf::Mouse::Left) {
                        if (event.mouseButton.x >= 10 && event.mouseButton.x <= 210) {
                            if (event.mouseButton.y >= 10 && event.mouseButton.y <= 60) {
                                ...
                                performDelaunayTriangulation(filePoints, triangles);
                                std::cout << "Number of triangles: " << triangles.size() << std::endl;
                                // Now draw everything on the screen
                                drawEverything(window); // This will now draw the updated triangles as well
                                // Pause to visualize
